 ...
 */
	if ( ! class_exists( 'freemium_Plugin_Activation' ) ) {
    /*  Automatic plugin installation and activation library. */
    class freemium_Plugin_Activation {
        ...
        public $menu = 'freemium-install-plugins';
        ...
        public function __construct() {
            ...
            do_action_ref_array( 'freemium_init', array( $this ) );
        ...
        public function init() {
            // Load class strings.
            $this->strings = array(
                'page_title'                     => __( 'Install Required Plugins', 'freemium' ),
                'menu_title'                     => __( 'Install Plugins', 'freemium' ),
                'installing'                     => __( 'Installing Plugin: %s', 'freemium' ),
                'oops'                           => __( 'Something went wrong.', 'freemium' ),
                'notice_can_install_required'    => _n_noop( 'This theme requires the following plugin: %1$s.', 'This theme requires the following plugins: %1$s.', 'freemium' ),
                'notice_can_install_recommended' => _n_noop( 'This theme recommends the following plugin: %1$s.', 'This theme recommends the following plugins: %1$s.', 'freemium' ),
                'notice_cannot_install'          => _n_noop( 'Sorry, but you do not have the correct permissions to install the %s plugin. Contact the administrator of this site for help on getting the plugin installed.', 'Sorry, but you do not have the correct permissions to install the %s plugins. Contact the administrator of this site for help on getting the plugins installed.', 'freemium' ),
                'notice_can_activate_required'   => _n_noop( 'The following required plugin is currently inactive: %1$s.', 'The following required plugins are currently inactive: %1$s.', 'freemium' ),
                'notice_can_activate_recommended'=> _n_noop( 'The following recommended plugin is currently inactive: %1$s.', 'The following recommended plugins are currently inactive: %1$s.', 'freemium' ),
                'notice_cannot_activate'         => _n_noop( 'Sorry, but you do not have the correct permissions to activate the %s plugin. Contact the administrator of this site for help on getting the plugin activated.', 'Sorry, but you do not have the correct permissions to activate the %s plugins. Contact the administrator of this site for help on getting the plugins activated.', 'freemium' ),
                'notice_ask_to_update'           => _n_noop( 'The following plugin needs to be updated to its latest version to ensure maximum compatibility with this theme: %1$s.', 'The following plugins need to be updated to their latest version to ensure maximum compatibility with this theme: %1$s.', 'freemium' ),
                'notice_cannot_update'           => _n_noop( 'Sorry, but you do not have the correct permissions to update the %s plugin. Contact the administrator of this site for help on getting the plugin updated.', 'Sorry, but you do not have the correct permissions to update the %s plugins. Contact the administrator of this site for help on getting the plugins updated.', 'freemium' ),
                'install_link'                   => _n_noop( 'Begin installing plugin', 'Begin installing plugins', 'freemium' ),
                'activate_link'                  => _n_noop( 'Begin activating plugin', 'Begin activating plugins', 'freemium' ),
                'return'                         => __( 'Return to Required Plugins Installer', 'freemium' ),
                'dashboard'                      => __( 'Return to the dashboard', 'freemium' ),
                'plugin_activated'               => __( 'Plugin activated successfully.', 'freemium' ),
                'activated_successfully'         => __( 'The following plugin was activated successfully:', 'freemium' ),
                'complete'                       => __( 'All plugins installed and activated successfully. %1$s', 'freemium' ),
                'dismiss'                        => __( 'Dismiss this notice', 'freemium' ),
            );
            do_action( 'freemium_register' );
        ...
        public function thickbox() {
            if ( ! get_user_meta( get_current_user_id(), 'freemium_dismissed_notice_' . $this->id, true ) ) {
                add_thickbox();
            } }
        ...
        public function install_plugins_page() {
            ...
            $plugin_table = new freemiumPA_List_Table;
            // Return early if processing a plugin installation action.
            if ( isset( $_POST['action'] ) && 'freemium-bulk-install' == $_POST['action'] && $plugin_table->process_bulk_actions() || $this->do_plugin_install() ) {
                return;
            } ?>

<div class="freemium wrap">
  ...
  <form id="freemium-plugins" action="" method="post">
    <input type="hidden" name="freemium-page" value="<?php echo $this->menu; ?>" />
...
<?php
        ...
        protected function do_plugin_install() {
            ...
            if ( isset( $_GET['plugin'] ) && ( isset( $_GET['freemium-install'] ) && 'install-plugin' == $_GET['freemium-install'] ) ) {
                check_admin_referer( 'freemium-install' );
                ...
                $url = wp_nonce_url(
                    add_query_arg(
                        array(
                            ...
                            'freemium-install' => 'install-plugin',
                        ),
                ...
                $fields = array( 'freemium-install' ); // Extra fields to pass to WP_Filesystem.
                ...
                if ( empty( $complete ) ) {
                    echo '<p>' .  sprintf( $this->strings['complete'], '<a href="' . admin_url() . '" title="' . __( 'Return to the Dashboard', 'freemium' ) . '">' . __( 'Return to the Dashboard', 'freemium' ) . '</a>' ) . '</p>';
                    echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
                } return true;
            }
            // Checks for actions from hover links to process the activation.
            elseif ( isset( $_GET['plugin'] ) && ( isset( $_GET['freemium-activate'] ) && 'activate-plugin' == $_GET['freemium-activate'] ) ) {
                check_admin_referer( 'freemium-activate', 'freemium-activate-nonce' );
        ...
        public function notices() {
            ...
            if ( $this->is_freemium_page() ) { return; }
            // Return early if the nag message has been dismissed.
            if ( get_user_meta( get_current_user_id(), 'freemium_dismissed_notice_' . $this->id, true ) ) {  return; }
            ...
            if ( ! empty( $message ) ) {
                ...
                foreach ( $message as $type => $plugin_groups ) {
                    ...
                    $rendered .= '<p>' . sprintf( translate_nooped_plural( $this->strings[$type], $count, 'freemium' ), $imploded, $count ) . '</p>';
                }
                // Setup variables to determine if action links are needed.
                $show_install_link  = $install_link ? '<a href="' . add_query_arg( 'page', $this->menu, admin_url( 'themes.php' ) ) . '">' . translate_nooped_plural( $this->strings['install_link'], $install_link_count, 'freemium' ) . '</a>' : '';
                $show_activate_link = $activate_link ? '<a href="' . add_query_arg( 'page', $this->menu, admin_url( 'themes.php' ) ) . '">' . translate_nooped_plural( $this->strings['activate_link'], $activate_link_count, 'freemium' ) . '</a>'  : '';
                // Define all of the action links.
                $action_links = apply_filters(
                    'freemium_notice_action_links',
                    array(
                        'install'  => ( current_user_can( 'install_plugins' ) )  ? $show_install_link  : '',
                        'activate' => ( current_user_can( 'activate_plugins' ) ) ? $show_activate_link : '',
                        'dismiss'  => $this->dismissable ? '<a class="dismiss-notice" href="' . add_query_arg( 'freemium-dismiss', 'dismiss_admin_notices' ) . '" target="_parent">' . $this->strings['dismiss'] . '</a>' : '',
                    )
                );
                ...
                if ( ! empty( $this->strings['nag_type'] ) ) {
                    add_settings_error( 'freemium', 'freemium', $rendered, sanitize_html_class( strtolower( $this->strings['nag_type'] ) ) );
                } else {
                    add_settings_error( 'freemium', 'freemium', $rendered, $nag_class );
                } }
            // Admin options pages already output settings_errors, so this is to avoid duplication.
            if ( 'options-general' !== $current_screen->parent_base ) {
                settings_errors( 'freemium' );
            } }
        /* Add dismissable admin notices. */
        public function dismiss() {
            if ( isset( $_GET['freemium-dismiss'] ) ) {
                update_user_meta( get_current_user_id(), 'freemium_dismissed_notice_' . $this->id, 1 );
            } }
        ...
        public function actions( $install_actions ) {
            // Remove action links on the freemiumPA install page.
            if ( $this->is_freemium_page() ) { return false; }
        ...
        /* Determine if we're on the freemiumPA Install page. */
        protected function is_freemium_page() {
        ...
        public function update_dismiss() {
            delete_user_meta( get_current_user_id(), 'freemium_dismissed_notice_' . $this->id );
        }
        ...
        public static function get_instance() {
            if ( ! isset( self::$instance ) && ! ( self::$instance instanceof freemium_Plugin_Activation ) ) {
                self::$instance = new freemium_Plugin_Activation();
            } return self::$instance;
        } }
    // Ensure only one instance of the class is ever invoked.
    $freemium = freemium_Plugin_Activation::get_instance();
}
if ( ! function_exists( 'freemium' ) ) {
    /* Helper function to register a collection of required plugins. */
    function freemium( $plugins, $config = array() ) {
        foreach ( $plugins as $plugin ) {
            freemium_Plugin_Activation::$instance->register( $plugin );
        } if ( $config ) {
            freemium_Plugin_Activation::$instance->config( $config );
        } } }
/* WP_List_Table isn't always available. If it isn't available, we load it here. */
if ( ! class_exists( 'WP_List_Table' ) ) { require_once( ABSPATH . 'wp-admin/includes/class-wp-list-table.php' );
}
if ( ! class_exists( 'freemiumPA_List_Table' ) ) {
    /* List table class for handling plugins. */
    class freemiumPA_List_Table extends WP_List_Table {
        ...
        protected function _gather_plugin_data() {
            ...
            freemium_Plugin_Activation::$instance->admin_init();
            freemium_Plugin_Activation::$instance->thickbox();
            // Prep variables for use and grab list of all installed plugins.
            $table_data        = array();
            $i                 = 0;
            $installed_plugins = get_plugins();
            foreach ( freemium_Plugin_Activation::$instance->plugins as $plugin ) {
                if ( is_plugin_active( $plugin['file_path'] ) ) {
                ...
                if ( ! empty( $plugin['source'] ) ) {
                    ...
                    if ( preg_match( '|^http(s)?://|', $plugin['source'] ) ) {
                        $table_data[$i]['source'] = __( 'Private Repository', 'freemium' );
                    // The plugin is pre-packaged with the theme.
                    } else {
                        $table_data[$i]['source'] = __( 'Pre-Packaged', 'freemium' );
                    } } else {
                    $table_data[$i]['source'] = __( 'WordPress Repository', 'freemium' );
                }
                $table_data[$i]['type'] = isset( $plugin['required'] ) && $plugin['required'] ? __( 'Required', 'freemium' ) : __( 'Recommended', 'freemium' );
                if ( ! isset( $installed_plugins[$plugin['file_path']] ) ) {
                    $table_data[$i]['status'] = sprintf( '%1$s', __( 'Not Installed', 'freemium' ) );
                } elseif ( is_plugin_inactive( $plugin['file_path'] ) ) {
                    $table_data[$i]['status'] = sprintf( '%1$s', __( 'Installed But Not Activated', 'freemium' ) );
                }
        ...
        /* Retrieve plugin data, given the plugin name. Taken from the freemium_Plugin_Activation class.*/
        protected function _get_plugin_data_from_name( $name, $data = 'slug' ) {
            foreach ( freemium_Plugin_Activation::$instance->plugins as $plugin => $values ) {
                if ( $name == $values['name'] && isset( $values[$data] ) ) {
                    return $values[$data];
                } } return false;
        }
        ...
        public function column_plugin( $item ) {
            ...
            if ( ! isset( $installed_plugins[$item['file_path']] ) ) {
                $actions = array(
                    'install' => sprintf(
                        '<a href="%1$s" title="' . __( 'Install', 'freemium' ) . ' %2$s">' . __( 'Install', 'freemium' ) . '</a>',
                        wp_nonce_url(
                            add_query_arg(
                                array(
                                    'page'          => freemium_Plugin_Activation::$instance->menu,
                                    'plugin'        => $item['slug'],
            ...
            elseif ( is_plugin_inactive( $item['file_path'] ) ) {
                $actions = array(
                    'activate' => sprintf(
                        '<a href="%1$s" title="' . __( 'Activate', 'freemium' ) . ' %2$s">' . __( 'Activate', 'freemium' ) . '</a>',
                        add_query_arg(
                            array(
                                'page'                 => freemium_Plugin_Activation::$instance->menu,
                                'plugin'               => $item['slug'],
                                'plugin_name'          => $item['sanitized_plugin'],
                                'plugin_source'        => $item['url'],
                                'freemium-activate'       => 'activate-plugin',
                                'freemium-activate-nonce' => wp_create_nonce( 'freemium-activate' ),
                            ), admin_url( 'themes.php' )
                        ), $item['sanitized_plugin']
                    ), );
            }
        ...
        public function no_items() {              
            _e('No plugins to install or activate','freemium'); echo ".<a href='admin_url()'>".__('Return to the Dashboard','freemium'). "</a>";
        ...
        public function get_columns() {
            $columns = array(
                ...
                'plugin' => __( 'Plugin', 'freemium' ),
                'source' => __( 'Source', 'freemium' ),
                'type'   => __( 'Type', 'freemium' ),
                'status' => __( 'Status', 'freemium' )
            ); return $columns;
        }
        /* Defines all types of bulk actions for handling*/
        public function get_bulk_actions() {
            $actions = array(
                'freemium-bulk-install'  => __( 'Install', 'freemium' ),
                'freemium-bulk-activate' => __( 'Activate', 'freemium' ),
            );
            return $actions;
        }
        ...
        public function process_bulk_actions() {
            ...
            if ( 'freemium-bulk-install' === $this->current_action() ) {
                ...
                if ( empty( $plugin_installs ) ) {
                    echo '<div id="message" class="error"><p>' . __( 'No plugins are available to be installed at this time.', 'freemium' ) . '</p></div>';
                    return false;
                }
                ...
                $url = wp_nonce_url(
                    add_query_arg(
                        array(
                            'page'          => freemium_Plugin_Activation::$instance->menu,
                            'freemium-action'  => 'install-selected',
                ...
                $sources      = array();
                $install_path = array();
                // Loop through each plugin to install and try to grab information from WordPress API, if not create 'freemium-empty' scalar.
                $i = 0;
                foreach ( $plugin_installs as $plugin ) {
                    $api[$i] = plugins_api( 'plugin_information', array( 'slug' => $plugin, 'fields' => array( 'sections' => false ) ) ) ? plugins_api( 'plugin_information', array( 'slug' => $plugin, 'fields' => array( 'sections' => false ) ) ) : (object) $api[$i] = 'freemium-empty';
                    $i++;
                }
                if ( is_wp_error( $api ) ) { wp_die( freemium_Plugin_Activation::$instance->strings['oops'] . var_dump( $api ) ); }
                ...
                $url   = add_query_arg( array( 'page' => freemium_Plugin_Activation::$instance->menu ), admin_url( 'themes.php' ) );
                $nonce = 'bulk-plugins';
                $names = $plugin_names;
                // Create a new instance of freemium_Bulk_Installer.
                $installer = new freemium_Bulk_Installer( $skin = new freemium_Bulk_Installer_Skin( compact( 'url', 'nonce', 'names' ) ) );
                // Wrap the install process with the appropriate HTML.
                echo '<div class="freemium wrap">';
                    if ( version_compare( freemium_Plugin_Activation::$instance->wp_version, '3.8', '<' ) ) {
                    }
            ...
            if ( 'freemium-bulk-activate' === $this->current_action() ) {
                check_admin_referer( 'bulk-' . $this->_args['plural'] );
                ...
                if ( empty( $plugins_to_activate ) ) {
                    echo '<div id="message" class="error"><p>' . __( 'No plugins are available to be activated at this time.', 'freemium' ) . '</p></div>';
                    return false;
                }
                $plugins      = array();
                $plugin_names = array();
                ...
                if ( is_wp_error( $activate ) ) {
                    ...
                    printf( '<div id="message" class="updated"><p>%1$s %2$s.</p></div>', _n( 'The following plugin was activated successfully:', 'The following plugins were activated successfully:', $count, 'freemium' ), $imploded );
                }
 ...
 */
add_action( 'admin_init', 'freemium_load_bulk_installer' );
function freemium_load_bulk_installer() {
    if ( ! class_exists( 'WP_Upgrader' ) && ( isset( $_GET['page'] ) && freemium_Plugin_Activation::$instance->menu === $_GET['page'] ) ) {
        require_once ABSPATH . 'wp-admin/includes/class-wp-upgrader.php';
        if ( ! class_exists( 'freemium_Bulk_Installer' ) ) {
            /* Installer class to handle bulk plugin installations. */
            class freemium_Bulk_Installer extends WP_Upgrader {
                ...
                public function bulk_install( $packages ) {
                    ...
                    if ( freemium_Plugin_Activation::$instance->is_automatic ) { $this->activate_strings(); }
                ...
                public function run( $options ) {
                    ...
                    if ( freemium_Plugin_Activation::$instance->is_automatic ) {
                        // Flush plugins cache so we can make sure that the installed plugins list is always up to date.
                        wp_cache_flush();
                        ...
                        freemium_Plugin_Activation::$instance->populate_file_path();
                        // Set correct strings based on results.
                        if ( is_wp_error( $activate ) ) {
                ...
                public function install_strings() {
                    $this->strings['no_package']          = __( 'Install package not available.', 'freemium' );
                    $this->strings['downloading_package'] = __( 'Downloading install package from <span class="code">%s</span>&#8230;', 'freemium' );
                    $this->strings['unpack_package']      = __( 'Unpacking the package&#8230;', 'freemium' );
                    $this->strings['installing_package']  = __( 'Installing the plugin&#8230;', 'freemium' );
                    $this->strings['process_failed']      = __( 'Plugin install failed.', 'freemium' );
                    $this->strings['process_success']     = __( 'Plugin installed successfully.', 'freemium' );
                }
                /* Sets the correct activation strings for the installer skin to use.*/
                public function activate_strings() {
                    $this->strings['activation_failed']  = __( 'Plugin activation failed.', 'freemium' );
                    $this->strings['activation_success'] = __( 'Plugin activated successfully.', 'freemium' );
                }
        ...
        if ( ! class_exists( 'freemium_Bulk_Installer_Skin' ) ) {
            /* Installer skin to set strings for the bulk plugin installations. */
            class freemium_Bulk_Installer_Skin extends Bulk_Upgrader_Skin {
                ...
                public function add_strings() {
                    ...
                    if ( freemium_Plugin_Activation::$instance->is_automatic ) {
                        $this->upgrader->strings['skin_upgrade_start']        = __( 'The installation and activation process is starting. This process may take a while on some hosts, so please be patient.', 'freemium' );
                        $this->upgrader->strings['skin_update_successful']    = __( '%1$s installed and activated successfully.', 'freemium' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', 'freemium' ) . '</span><span class="hidden">' . __( 'Hide Details', 'freemium' ) . '</span>.</a>';
                        $this->upgrader->strings['skin_upgrade_end']          = __( 'All installations and activations have been completed.', 'freemium' );
                        $this->upgrader->strings['skin_before_update_header'] = __( 'Installing and Activating Plugin %1$s (%2$d/%3$d)', 'freemium' );
                    }
                    // Default installation strings.
                    else {
                        $this->upgrader->strings['skin_upgrade_start']        = __( 'The installation process is starting. This process may take a while on some hosts, so please be patient.', 'freemium' );
                        $this->upgrader->strings['skin_update_failed_error']  = __( 'An error occurred while installing %1$s: <strong>%2$s</strong>.', 'freemium' );
                        $this->upgrader->strings['skin_update_failed']        = __( 'The installation of %1$s failed.', 'freemium' );
                        $this->upgrader->strings['skin_update_successful']    = __( '%1$s installed successfully.', 'freemium' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', 'freemium' ) . '</span><span class="hidden">' . __( 'Hide Details', 'freemium' ) . '</span>.</a>';
                        $this->upgrader->strings['skin_upgrade_end']          = __( 'All installations have been completed.', 'freemium' );
                        $this->upgrader->strings['skin_before_update_header'] = __( 'Installing Plugin %1$s (%2$d/%3$d)', 'freemium' );
                    }
                }
                ...
                public function bulk_footer() {
                    ...
                    foreach ( freemium_Plugin_Activation::$instance->plugins as $plugin ) {
                        if ( ! is_plugin_active( $plugin['file_path'] ) ) {
                            echo '<p><a href="' . add_query_arg( 'page', freemium_Plugin_Activation::$instance->menu, admin_url( 'themes.php' ) ) . '" title="' . esc_attr( freemium_Plugin_Activation::$instance->strings['return'] ) . '" target="_parent">' . freemium_Plugin_Activation::$instance->strings['return'] . '</a></p>';
                            $complete[] = $plugin;
                            break;
                        } else {
                            $complete[] = '';
                    ...
                    if ( empty( $complete ) ) {
                        echo '<p>' .  sprintf( freemium_Plugin_Activation::$instance->strings['complete'], '<a href="' . admin_url() . '" title="' . __( 'Return to the Dashboard', 'freemium' ) . '">' . __( 'Return to the Dashboard', 'freemium' ) . '</a>' ) . '</p>';
                        echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
                    } }
                /* Flush header output buffer. */
